(() => {
  ...
  async function saveSettings(v){const payload={id:1,storeName:v.storeName||'ATP Suplementos',whatsapp:v.whatsapp||'',instagram:v.instagram||'',location:v.location||'',heroTitle:v.heroTitle||'',heroText:v.heroText||'',maintenanceMode:Boolean(v.maintenanceMode),maintenanceMessage:v.maintenanceMessage||'Estamos agregando y actualizando productos. En breve la tienda volverá a estar disponible.',updated_at:new Date().toISOString()};const{data,error}=await client.from('atp_settings').upsert(payload,{onConflict:'id'}).select().single();if(error)throw error;return data}
