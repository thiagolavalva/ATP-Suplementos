...
function renderProducts(){
  ...
  const list=products.filter(p=>(filter==="Todos"||p.category===filter)&&`${p.name} ${p.brand} ${p.category}`.toLowerCase().includes(q));
...
function suggestionsRender(){const q=searchInput.value.trim().toLowerCase();if(!q){suggestions.innerHTML="";return}const list=products.filter(p=>`${p.name} ${p.brand}`.toLowerCase().includes(q)).slice(0,6);suggestions.innerHTML=list.map(p=>`<div class="suggestion" data-id="${p.id}"><b>${p.name}</b><span>${p.brand} · ${money(p.price)}</span></div>`).join("");suggestions.querySelectorAll(".suggestion").forEach(s=>s.onclick=()=>location.href=`product.html?id=${encodeURIComponent(s.dataset.id)}`)}
...
searchOpen.onclick=openSearch;searchClose.onclick=closeSearch;searchInput.oninput=()=>{query=searchInput.value;suggestionsRender();renderProducts()};
