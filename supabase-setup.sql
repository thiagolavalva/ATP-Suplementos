...
-- ATP Suplementos v1.4: DNI y eliminación segura de clientes
...
begin
 ...
 on conflict(phone) do update set
   name=excluded.name,
   email=case when excluded.email<>'' then excluded.email else atp_customers.email end,
   tax_id=case when excluded.tax_id<>'' then excluded.tax_id else atp_customers.tax_id end,
   orders_count=atp_customers.orders_count+1,
   total_spent=atp_customers.total_spent+excluded.total_spent,
   points_balance=atp_customers.points_balance+excluded.points_balance,
   lifetime_points=atp_customers.lifetime_points+excluded.lifetime_points,
   last_order_at=now(),is_active=true,deleted_at=null,updated_at=now()
 returning * into c;
 update public.atp_coupons set used_count=used_count+1,updated_at=now() where code=o.coupon and o.coupon<>'';
 update public.atp_orders set payment_status='approved',status='pago_confirmado',mp_payment_id=p_payment_id,stock_processed=true,points_awarded=pts,customer_id=c.id,tracking_code=coalesce(tracking_code,'ATP-'||upper(substr(replace(id::text,'-',''),1,8))),updated_at=now() where id=o.id;
 return jsonb_build_object('ok',true,'points',pts,'tracking_code',coalesce(o.tracking_code,'ATP-'||upper(substr(replace(o.id::text,'-',''),1,8))));
