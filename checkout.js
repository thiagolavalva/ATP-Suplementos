...
function entries(){return Object.entries(cart).map(([key,q])=>({...splitCartKey(key),key,quantity:Number(q)})).filter(e=>products.some(p=>String(p.id)===e.productId))}
function totals(){const subtotalValue=entries().reduce((sum,e)=>{const p=products.find(x=>String(x.id)===e.productId);return sum+Number(p.price)*e.quantity},0),percent=Number(coupons[appliedCoupon]||0),discountValue=Math.round(subtotalValue*percent)/100;return{subtotal:subtotalValue,discount:discountValue,total:Math.max(0,subtotalValue-discountValue)}}
function renderSummary(){const list=entries();if(!list.length){location.replace('index.html');return}orderItems.innerHTML=list.map(e=>{const p=products.find(x=>String(x.id)===e.productId),v=variantsOf(p).find(x=>String(x.id)===e.variantId);return`<div class="order-item"><div><h3>${e.quantity} × ${p.name}</h3><small>${p.brand||'ATP Suplementos'}${v?` · ${v.name}`:''}</small></div><b>${money(Number(p.price)*e.quantity)}</b></div>`}).join('');const t=totals();subtotal.textContent=money(t.subtotal);discount.textContent=`-${money(t.discount)}`;total.textContent=money(t.total);if(window.totalHighlight)totalHighlight.textContent=money(t.total);discountRow.classList.toggle('visible',t.discount>0)}
